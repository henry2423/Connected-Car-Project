...
from findingline.main import process
...
def function_process_image(image):
    return image
            ...
            for t in bag_img_dic['left'].keys():
                ...
                if t in L:
                    ...
                    if rt in bag_img_dic['right']:
                        left_images_[t] = function_process_image(bag_img_dic['left'][t])
                        right_images_[t] = function_process_image(bag_img_dic['right'][rt])
                    else:
                        pass
...
def function_write_video(vrimage, limage, steer, motor, state, i, j):
    # image = limage.astype(np.uint8)
    mix = motor + steer
    cv2.putText(limage, 'steer: ' + str(steer), (10,40), cv2.FONT_HERSHEY_SIMPLEX, 1, 
    (255, 0, 255), 1, cv2.LINE_AA)
    cv2.putText(limage, 'motor: ' + str(motor), (10,80), cv2.FONT_HERSHEY_SIMPLEX, 1, 
    (0, 255, 255), 1, cv2.LINE_AA)
    cv2.putText(limage, 'state: ' + str(state), (10,120), cv2.FONT_HERSHEY_SIMPLEX, 1, 
    (0, 255, 255), 1, cv2.LINE_AA)
    scipy.misc.imsave('video/image/' + str(i) +  '/test'+str(j)+'.jpg', limage)
    #r,g,b = image
    #test_image = np.dstack([r,g,b])
    # video_out.write(cv2.imread('video/test.jpg'))
...
def function_save_hdf5(run_num=None, dst_path=opj(bair_car_data_path, 'hdf5/runs'), flip=False):
    ...
    for i in range(len(segment_list_with_min_len)):
        ...
        motor_list = []
        state_list = []
        unix('mkdir -p '+'video/image/' + str(i)+'/')
        for j in range(len(segment)):
            ...
            #function_write_video( rimg, limg, I[steer][t], I[motor][t], I[state][t], i, j)
...
def load_hdf5_steer_hist(path, dst_path):
    ...
    try:
        ...
        for h in range(len(s)): 
            ...
            for i in range(len(s[n][left])):
                function_write_video(s[n][state][i], s[n][left][i], s[n][steer][i], s[n][motor][i], s[n][state][i], n, i)
