...
def process(image, ret, mtx, dist, rvecs, tvecs):
    cv2.resize(image, (resize_w, resize_h))
    frame = calibrate_image(image, ret, mtx, dist, rvecs, tvecs)
    ...
    frame_undistorted = cv2.undistort(image, mtx, dist, newCameraMatrix=mtx)
    return draw_lines(frame_undistorted, warped, left_fit, right_fit,  left_fitx, right_fitx , ploty)
